...
export class PostCommentService {
  ...
  public async getCommentsOfPost(postId: string, page: number, size: number, requestedBy: string) {
    ...
    const result = await queryBuilder
      ...
      .leftJoinAndMapOne('user.isBlocks', 'user.blocks', 'userBlocks', 'userBlocks.blocks.id = :userId', { requestedBy })
      .leftJoinAndMapOne('user.isBlocked', 'user.blockedBy', 'userBlocked', 'userBlocked.blockedBy.id = :userId', { requestedBy })
